...
class NLayout : LinearLayout, NestedScrollingParent2, NestedScrollingChild2 {
...
//        onNestedScrollInternal(target, dyUnconsumed, type)
//        if (dyUnconsumed < 0) {
//            //对于向下滑动
...
//        }
    ...
    private fun onNestedScrollInternal(target: View, dy: Int, type: Int) {
        //对于底部布局
        val hideBottom = dy < 0 && scrollY > 0
        val showBottom = (dy > 0 && !target.canScrollVertically(1)
                && !(topView?.canScrollVertically(1) ?: true)) && scrollY < bottomViewHeight
